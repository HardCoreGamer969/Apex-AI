...
from backend.services.f1_adapter import get_lap_data
...
def get_lap(
    ...
    cache_key = f"lap:{year}:{round_number}:{session}:{driver.upper()}:{lap}"
    cached = l1_get(cache_key)
    if cached is not None:
        logger.info("Lap cache hit: %s", cache_key)
        return Response(content=orjson.dumps(cached), media_type="application/json")
    try:
        data = get_lap_data(year=year, round_number=round_number, session_type=session, driver=driver.upper(), lap_number=lap)
